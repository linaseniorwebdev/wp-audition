 ...
 * @since 7.0
 */
function audition_ms_register_default_actions() {
	audition_register_action( 'signup', array(
		'title'             => '',
		'slug'              => 'signup',
		'callback'          => 'audition_ms_signup_handler',
		'network'           => true,
		'show_on_forms'     => false,
	...
	audition_register_action( 'activate', array(
		'title'             => '',
		'slug'              => 'activate',
		'callback'          => 'audition_ms_activation_handler',
		'network'           => true,
		'show_on_forms'     => false,
		'show_in_widget'    => false,
		'show_in_nav_menus' => false,
 ...
 * @since 7.0
 */
function audition_ms_register_default_forms() {
	audition_ms_register_user_signup_form();
	audition_ms_register_blog_signup_form();
	audition_ms_register_another_blog_signup_form();
	audition_ms_register_activation_form();
}
 ...
 * @since 7.0
 */
function audition_ms_register_user_signup_form() {

	audition_register_form( 'user_signup', array(
		'action'      => audition_get_action_url( 'signup' ),
		'attributes'  => array(
			'id'         => 'setupform',
	...
	audition_add_form_field( 'user_signup', 'stage', array(
		'type'     => 'hidden',
		'value'    => 'validate-user-signup',
		'priority' => 5,
	) );

	audition_add_form_field( 'user_signup', 'signup_hidden_fields', array(
		'type'        => 'action',
		'render_args' => array( 'validate-user' ),
	) );

	if ( audition_is_default_registration_type() ) {
		audition_add_form_field( 'user_signup', 'user_name', array(
			'type'        => 'text',
			'label'       => __( 'Username:' ),
	...
	audition_add_form_field( 'user_signup', 'user_email', array(
		'type'        => 'email',
	...
	audition_add_form_field( 'user_signup', 'signup_extra_fields', array(
		'type'        => 'action',
		'render_args' => array( audition_get_errors() ),
	) );

	$active_signup = audition_ms_signup_get_active_signup();

	if ( 'blog' == $active_signup ) {
		audition_add_form_field( 'user_signup', 'signup_for', array(
			'type'     => 'hidden',
			'value'    => 'blog',
			'id'       => 'signupblog',
			'priority' => 20,
	...
	audition_add_form_field( 'user_signup', 'submit', array(
		'type'     => 'submit',
		'name'     => 'submit',
		'value'    => __( 'Next' ),
		'priority' => 25,
 ...
 * @since 7.0
 */
function audition_ms_register_blog_signup_form() {

	audition_register_form( 'blog_signup', array(
		'action'      => audition_get_action_url( 'signup' ),
		'attributes'  => array(
			'id' => 'setupform',
		),
	...
	audition_add_form_field( 'blog_signup', 'stage', array(
		'type'     => 'hidden',
		'value'    => 'validate-blog-signup',
		'priority' => 5,
	) );
	audition_add_form_field( 'blog_signup', 'user_name', array(
		'type'     => 'hidden',
		'priority' => 5,
	) );
	audition_add_form_field( 'blog_signup', 'user_email', array(
		'type'     => 'hidden',
		'priority' => 5,
	) );

	audition_add_form_field( 'blog_signup', 'signup_hidden_fields', array(
		'type'        => 'action',
		'render_args' => array( 'validate-site' ),
	) );

	audition_ms_add_blog_signup_form_fields( 'blog_signup' );

	audition_add_form_field( 'blog_signup', 'submit', array(
		'type'     => 'submit',
		'value'    => __( 'Signup' ),
		'priority' => 30,
	) );
 ...
 * @since 7.0
 */
function audition_ms_register_another_blog_signup_form() {

	audition_register_form( 'another_blog_signup', array(
		'action'      => audition_get_action_url( 'signup' ),
		'attributes'  => array(
			'id' => 'setupform',
		),
	...
	audition_add_form_field( 'another_blog_signup', 'stage', array(
		'type'     => 'hidden',
		'value'    => 'gimmeanotherblog',
		'priority' => 5,
	) );

	audition_add_form_field( 'another_blog_signup', 'signup_hidden_fields', array(
		'type'        => 'action',
		'render_args' => array( 'create-another-site' ),
	) );

	audition_ms_add_blog_signup_form_fields( 'another_blog_signup' );

	audition_add_form_field( 'another_blog_signup', 'submit', array(
		'type'     => 'submit',
 ...
 */
function audition_ms_add_blog_signup_form_fields( $form ) {
	...
	if ( ! is_subdomain_install() ) {
		...
		audition_add_form_field( $form, 'blogname', array(
			'type'        => 'text',
			'label'       => __( 'Site Name: '),
			'value'       => '',
			'id'          => 'blogname',
	...
	audition_add_form_field( $form, 'blog_title', array(
		'type'     => 'text',
		'label'    => __( 'Site Title:' ),
		'id'       => 'blog_title',
		'priority' => 15,
	...
	if ( $language_field = audition_ms_render_blog_signup_language_field() ) {
		audition_add_form_field( $form, 'site_language', array(
			'type'     => 'custom',
	...
	audition_add_form_field( $form, 'blog_public', array(
		'type'        => 'radio-group',
		'label'       => __( 'Privacy:' ),
	...
	audition_add_form_field( $form, 'signup_blogform', array(
		'type'        => 'action',
		'render_args' => array( audition_get_errors() ),
	) );
}
 ...
 * @since 7.0
 */
function audition_ms_register_activation_form() {

	audition_register_form( 'activate', array(
		'action'      => audition_get_action_url( 'activate' ),
		'attributes'  => array(
			'id'         => 'activateform',
			'novalidate' => 'novalidate',
		),
	...
	$key = audition_get_request_value( 'key' );

	audition_add_form_field( 'activate', 'key', array(
		'type'     => audition_allow_user_passwords() && $key ? 'hidden' : 'text',
		'label'    => audition_allow_user_passwords() && $key ? ''       : __( 'Activation Key:' ),
		'value'    => $key,
		'id'       => 'key',
		'priority' => 5,
	) );

	if ( audition_allow_user_passwords() && $key ) {
		audition_add_form_field( 'activate', 'user_pass1', array(
			'type'       => 'password',
			'label'      => __( 'Password' ),
			'id'         => 'pass1',
			'attributes' => array(
		...
		audition_add_form_field( 'activate', 'user_pass2', array(
			'type'       => 'password',
		...
		audition_add_form_field( 'activate', 'indicator', array(
			'type'     => 'custom',
			'content'  => '<div id="pass-strength-result" class="hide-if-no-js" aria-live="polite">' . __( 'Strength indicator' ) . '</div>',
			'priority' => 10,
		) );

		audition_add_form_field( 'activate', 'indicator_hint', array(
			'type'     => 'custom',
	...
	audition_add_form_field( 'activate', 'submit', array(
		'type'     => 'submit',
		'value'    => __( 'Activate' ),
		'priority' => 15,
	) );
 ...
 * @since 7.0
 */
function audition_ms_signup_handler() {
	...
	switch ( $stage ) {
		case 'validate-user-signup':
			if ( $active_signup == 'all' || $_POST['signup_for'] == 'blog' && $active_signup == 'blog' || $_POST['signup_for'] == 'user' && $active_signup == 'user' ) {
				...
				audition_set_data( 'signup_result', $result );

				if ( $errors->get_error_code() ) {
					audition_set_data( 'signup_form', 'user' );
					return;
				}

				if ( 'blog' == $_POST['signup_for'] ) {
					audition_set_data( 'signup_form', 'blog' );
					return;
				}
		...
		case 'validate-blog-signup':
			if ( $active_signup == 'all' || $active_signup == 'blog' ) {
				...
				audition_set_data( 'signup_user_result', $user_result );
				...
				if ( ! empty( $_POST['site_language'] ) ) {
					$languages = audition_ms_signup_get_available_languages();
		...
		case 'gimmeanotherblog':
			...
			if ( $errors->get_error_code() ) {
				audition_set_data( 'signup_form', 'another_blog' );
				return;
			}
			...
			audition_set_data( 'signup_blog_id', $blog_id );
			break;
 ...
 */
function audition_ms_filter_signup_shortcode( $content = '', $action = 'signup', $atts = array() ) {
	if ( 'signup' != $action ) {
		return $content;
	}
	...
	if ( current_user_can( 'manage_network' ) ) {
		$content .= '<div class="audition-messages">' . __( 'Greetings Network Administrator!' ) . ' ';
	...
	if ( $active_signup == 'none' ) {
		...
		switch ( $stage ) {
			case 'validate-user-signup':
				if ( $active_signup == 'all' || $_POST['signup_for'] == 'blog' && $active_signup == 'blog' || $_POST['signup_for'] == 'user' && $active_signup == 'user' ) {
					$result = audition_get_data( 'signup_result' );
					$form   = audition_get_data( 'signup_form' );
					if ( 'user' == $form ) {
						$content .= audition_ms_get_user_signup_form( $result['user_name'], $result['user_email'], $result['errors'] );
					} elseif ( 'blog' == $form ) {
						$content .= audition_ms_get_blog_signup_form( $result['user_name'], $result['user_email'] );
					} else {
						$content .= '<h2>' . sprintf( __( '%s is your new username' ), audition_is_email_login_type() ? $result['user_email'] : $result['user_name'] ) . '</h2>';
			...
			case 'validate-blog-signup':
				if ( $active_signup == 'all' || $active_signup == 'blog' ) {
					$user_result = audition_get_data( 'signup_user_result' );
					$blog_result = audition_get_data( 'signup_result' );
					$form        = audition_get_data( 'signup_form' );
					if ( 'user' == $form ) {
						$content .= audition_ms_get_user_signup_form( $user_result['user_name'], $user_result['user_email'], $user_result['errors'] );
					} elseif ( 'blog' == $form ) {
						$content .= audition_ms_get_blog_signup_form( $user_result['user_name'], $user_result['user_email'], $blog_result['blogname'], $blog_result['blog_title'], $blog_result['errors'] );
					} else {
			...
			case 'gimmeanotherblog':
				...
				if ( 'another_blog' == $form ) {
					$content .= audition_ms_get_another_blog_signup_form( $result['blogname'], $result['blog_title'], $result['errors'] );
				} else {
					$blog_id = audition_get_data( 'signup_blog_id' );
			...
			default:
				...
				if ( is_user_logged_in() && ( $active_signup == 'all' || $active_signup == 'blog' ) ) {
					$content .= audition_ms_get_another_blog_signup_form( $newblogname );
				} elseif ( ! is_user_logged_in() && ( $active_signup == 'all' || $active_signup == 'user' ) ) {
					$content .= audition_ms_get_user_signup_form( $newblogname, $user_email );
 ...
 */
function audition_ms_get_user_signup_form( $user_name = '', $user_email = '', $errors = '' ) {
	...
	$form = audition_get_form( 'user_signup' );
 ...
 */
function audition_ms_get_blog_signup_form( $user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '' ) {
	if ( ! is_wp_error( $errors ) ) {
		$errors  = new WP_Error();
	}
	...
	if ( audition_is_default_registration_type() && empty( $blogname ) ) {
		$blogname = $user_name;
	}

	$form = audition_get_form( 'blog_signup' );
 ...
 * @return string The blog signup form.
 */
function audition_ms_get_another_blog_signup_form( $blogname = '', $blog_title = '', $errors = '' ) {
	$current_user = wp_get_current_user();
	...
	$form = audition_get_form( 'another_blog_signup' );
 ...
 */
function audition_ms_render_blog_signup_language_field() {
	$markup = '';
 ...
 */
function audition_ms_signup_get_available_languages() {
	/** This filter is documented in wp-signup.php */
 ...
 */
function audition_ms_signup_get_active_signup() {
	/** This filter is documented in wp-signup.php */
 ...
 * @since 7.0
 */
function audition_ms_activation_handler() {
	...
	if ( ! empty( $_GET['key'] ) || ! empty( $_POST['key'] ) ) {
		...
		if ( audition_allow_user_passwords() ) {
			$errors = audition_validate_new_user_password();
			if ( $errors->get_error_code() ) {
				audition_set_data( 'activation_result', $errors );
				return;
			}
		}
		...
		if ( ! is_wp_error( $result ) ) {
			 ...
			 */
			$activation_redirect = apply_filters( 'audition_ms_activation_redirect', $activation_redirect, $result['user_id'], isset( $result['blog_id'] ) ? $result['blog_id'] : null );
		...
		audition_set_data( 'activation_result', $result );
	}
}
 ...
 * @return string The signup content if $action is 'signup' or the original content otherwise.
 */
function audition_ms_filter_activation_shortcode( $content = '', $action = 'signup', $atts = array() ) {
	if ( 'activate' != $action ) {
		return $content;
	}
	...
	if ( empty( $_GET['key'] ) && empty( $_POST['key'] ) ) {
		$content .= audition_get_form( 'activate' )->render( array(
			'before' => '<h2>' . __( 'Activation Key Required' ) . '</h2>',
		) );
	} else {
		$result = audition_get_data( 'activation_result' );
		if ( is_wp_error( $result ) ) {
			if ( 'already_active' == $result->get_error_code() || 'blog_taken' == $result->get_error_code() ) {
				...
				$content .= '</p>';
			} elseif ( audition_allow_user_passwords() && strpos( $result->get_error_code(), 'password' ) !== false ) {
				$form = audition_get_form( 'activate' );
			...
			$content .= '<p><span class="h3">' . __( 'Username:' ) . '</span> ' . ( audition_is_default_registration_type() ? $user->user_login : $user->user_email ) . '</p>';
			$content .= '<p><span class="h3">' . __( 'Password:' ) . '</span> ' . ( audition_allow_user_passwords() ? '****' : $result['password'] ) . '</p>';
			$content .= '</div>';
 ...
 */
function audition_ms_filter_pre_insert_user_data( $data = array() ) {
	if ( audition_allow_user_passwords() && ! empty( $_POST['user_pass1'] ) ) {
 ...
 */
function audition_ms_filter_welcome_email( $message, $blog_id, $user_id, $password ) {
	$user = get_userdata( $user_id );
	if ( audition_allow_user_passwords() && ! empty( $_POST['user_pass1'] ) ) {
		$message = str_replace( $password, $_POST['user_pass1'], $message );
	}
	if ( audition_is_email_login_type() ) {
 ...
 */
function audition_ms_filter_welcome_user_email( $message, $user_id, $password ) {
	$user = get_userdata( $user_id );
	if ( audition_allow_user_passwords() && ! empty( $_POST['user_pass1'] ) ) {
		$message = str_replace( 'PASSWORD', $_POST['user_pass1'], $message );
	}
